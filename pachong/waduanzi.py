...
def get_content(lists,page):
    if lists is not None:
        try:
            for l in lists:
                try:
                    title = l.find(class_="cd-title-link").attrs['title']
                    category = l.find(class_="post-author").find("a").get_text()
                    content = l.find(class_="item-content").prettify()
                    source = "http://www.waduanzi.com/"

                    Content.objects.create(title=title,category=category,content=content,source=source)
...
url="http://www.waduanzi.com/joke/page/%d"
